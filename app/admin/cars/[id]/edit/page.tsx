...
import { supabase, DEMO_CARS, type Car, type CarInsert } from '@/lib/supabase'
import Logo from '@/components/Logo'
import toast from 'react-hot-toast'

// ============================================================
// Admin — Formulaire de modification d'une voiture existante
// Pré-remplit tous les champs depuis les données existantes
// ============================================================
...
export default function EditCarPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [form, setForm] = useState<CarInsert | null>(null)
  ...
  // Vérifier l'auth + charger les données de la voiture
  useEffect(() => {
    async function init() {
      // Vérifier la session
      ...
      // Charger la voiture depuis Supabase
      try {
  ...
  const handleFeaturedChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setForm((prev) => prev ? ({ ...prev, featured: e.target.checked }) : null)
  }
    ...
    for (const file of files) {
      try {
        ...
        const { error } = await supabase.storage
          .from('car-images')
          .upload(`cars/${fileName}`, file, { cacheControl: '3600' })
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-[#141414] border border-[#1a1a1a] p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input name="model" type="text" required value={form.model} onChange={handleChange} className="form-input" />
              </div>
              <div>
                <label className="form-label">Year *</label>
                <input name="year" type="number" required min={1990} max={new Date().getFullYear() + 1} value={form.year} onChange={handleChange} className="form-input" />
              </div>
              <div>
                <label className="form-label">Mileage (km) *</label>
                <input name="mileage_km" type="number" required min={0} value={form.mileage_km} onChange={handleChange} className="form-input" />
              </div>
              <div>
                <label className="form-label">Color *</label>
                <input name="color" type="text" required value={form.color} onChange={handleChange} className="form-input" />
              </div>
              <div>
              ...
              <div className="sm:col-span-2">
                ...
                <input name="engine" type="text" required value={form.engine} onChange={handleChange} className="form-input" />
              </div>
              <div>
          ...
          {/* ===== PRIX ===== */}
          <div className="bg-[#141414] border border-[#1a1a1a] p-6">
            <h2 className="text-white font-semibold text-sm uppercase tracking-wider mb-6">Pricing</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="form-label">Japan Price (¥) *</label>
                <input name="price_jpy" type="number" required min={0} value={form.price_jpy} onChange={handleChange} className="form-input" />
              </div>
              <div>
                <label className="form-label">USD Estimate ($)</label>
                <input name="price_usd_estimate" type="number" min={0} value={form.price_usd_estimate} onChange={handleChange} className="form-input" />
              </div>
              <div>
                <label className="form-label">EUR Estimate (€)</label>
                <input name="price_eur_estimate" type="number" min={0} value={form.price_eur_estimate} onChange={handleChange} className="form-input" />
              </div>
            </div>
          </div>
          ...
          {/* ===== DESCRIPTION ===== */}
          <div className="bg-[#141414] border border-[#1a1a1a] p-6">
            <h2 className="text-white font-semibold text-sm uppercase tracking-wider mb-6">Description</h2>
            <textarea
          ...
          {/* ===== IMAGES ===== */}
          <div className="bg-[#141414] border border-[#1a1a1a] p-6">
            <h2 className="text-white font-semibold text-sm uppercase tracking-wider mb-6">Images</h2>
            ...
            <div className="mb-4">
              ...
              <label className="flex items-center gap-3 bg-[#111111] border border-dashed border-[#333] p-4 cursor-pointer hover:border-[#c9a96e]/40 transition-colors">
                ...
                <span className="text-[#a0a0a0] text-sm">
                  {uploadingFiles ? 'Uploading...' : 'Click to add more photos'}
                </span>
                <input type="file" multiple accept="image/*" onChange={handleFileUpload} className="hidden" disabled={uploadingFiles} />
              </label>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-2">
                ...
                />
                <button type="button" onClick={addImageUrl} className="btn-outline-gold py-3 px-4">
            ...
            {imageUrls.length > 0 && (
              <div className="space-y-2">
                ...
                {imageUrls.map((url, idx) => (
                  <div key={idx} className="flex items-center gap-3 bg-[#111111] border border-[#1a1a1a] p-2">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={url} alt="" className="w-16 h-10 object-cover shrink-0"
                      onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/80x50/141414/c9a96e?text=Error' }}
                    />
                    <span className="text-[#a0a0a0] text-xs flex-1 truncate">{url}</span>
                    <button type="button" onClick={() => removeImageUrl(idx)} className="text-[#666666] hover:text-red-400 transition-colors p-1">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                ))}
